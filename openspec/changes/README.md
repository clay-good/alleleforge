...
## Round 368 — the gate that was in neither

Last round's question, asked of the rest of the Makefile: which checks run somewhere the
signals a developer looks at cannot see? Most are covered twice — `lint`, `type`, `docs`
and `examples` are each a `make ci` step *and* a CI job, and `reproduce` joined the suite
last round. One was in neither.

`make figures` regenerates four SVGs into `docs/assets/figures/`. They are committed,
mkdocs serves them, the README and the preprint embed them, and that target is the only
thing that rewrites them. It is not in `make ci` and no CI job runs it, so nothing
compared the committed bytes to what the generator would produce.

That is the reproducibility golden's failure shape exactly, one artifact over: something
committed, something that generates it, nothing comparing the two. And it has already
cost this project once — a round that changed the SVG label layout regressed a five-bar
chart into hiding half its task names, and it surfaced only because someone regenerated
the figures by hand.

The figures are deterministic by construction, so the comparison is exact and takes half a
second. The suite now makes it, in both directions: committed bytes equal generated bytes,
and neither side holds a figure the other does not — an orphan in that directory is a file
nothing can regenerate, and a new figure nobody commits is a broken image in the built
docs.
