...
submission (task 3.2 — now shipped: `Submission.validate_admissible` rejects two
results for the same task), and the optional metric hardening (task 4). Only the
result/export `schema_version` (task 2) and the optional metric hardening (task 4)
remain.
