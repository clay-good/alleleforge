...
# the heavy pysam/cyvcf2/mappy chain is intentionally left out of the image. That
# reader now comes with the `web` extra itself — this image used to add it by hand,
# which meant `pip install "alleleforge[web]"` worked here and nowhere else.
...
COPY src ./src
RUN pip install ".[core,variant,cli,web]" "pyliftover>=0.4"
