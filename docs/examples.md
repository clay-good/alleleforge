...
Four runnable notebooks walk the AlleleForge journey end to end. Each is
...
| [`04_indel_prime_correction.ipynb`](https://github.com/clay-good/alleleforge/blob/main/examples/04_indel_prime_correction.ipynb) | **Correcting a small deletion** (ΔF508-shaped): routing admits prime and only prime, the **variable-length RT template** writes the missing bases back, and the RTT is read apart into homology + restored allele + homology. Shows why a deleted span costs no template length. |

## Running the notebooks

```bash
pip install -e ".[dev,cli]" "pyfaidx>=0.8" "pyliftover>=0.4"
pytest --nbmake examples/ --no-cov          # execute all four, as CI does
