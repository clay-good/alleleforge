...
!!! warning "Local, private, no egress"
    ...
    network call** and transmits **no sequence data externally** — a property asserted
    by a test that fails if any socket connects during a design request. The served
    frontend states this prominently and uses no third-party scripts. The one exception is **consequence annotation**, doubly opt-in: the operator
...
| `GET /api/jobs/{job_id}/result` | Render a **finished** job's result in any format its synchronous twin offers (`?format=` — a design job: `json\|menu\|html\|pdf\|tsv\|parquet`; a cohort job: `json\|tsv`). Nothing is recomputed; the job's own result is rendered. The async doors are the ones a client behind a proxy timeout is told to use, so without this the client whose run is long enough to need a job was the only one who could not have the PDF, the flat table, or the untruncated menu. `409` while the job is unfinished or failed (with the failure reason), `422` for a format that kind of job does not have, `404` once the result is no longer retained — the store keeps the most recent finished jobs and does not survive a restart, and the message says so, because a caller here has usually just watched the job finish. |
...
| `POST /api/batch` (response) | Carries `coordinate_system` beside the disclaimer, like every other response that returns a locus: each row's `variant` is the *resolved* one, printed 0-based, and a cohort is the artifact most likely to be forwarded to someone who did not make the request. |
