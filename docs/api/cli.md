...
| `aforge offtarget --cache` | Reuse a cross-run off-target report store. `--verbose` says how it was used — `off-target cache: 3 reused, 1 scan computed and stored` — because the result is byte-identical either way, so a key that stopped matching otherwise looks exactly like a warm cache. |
