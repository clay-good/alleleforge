...
| `aforge cache verify` | Check the four on-disk stores a run trusts: two holding *work* it reuses — the cross-run off-target report cache and the persistent FM-index cache. Each already knew how to detect a corrupted entry and neither could be *asked*: the report cache re-checks its checksum only when a run happens to read that entry, and the index's reconstruct-and-re-hash check — the only one that catches an alteration leaving the length intact, and so the only one that catches an index reporting positions that are not occurrences — was a Python method no shell could call. `--deep` runs it; without it the output says the indexes got their structural checks only, rather than reporting them as verified. Exits non-zero naming any entry that fails; nothing is repaired or deleted, since both stores are content-addressed and removing a named entry is always safe. The other two hold pinned *artifacts* — the dataset and checkpoint caches, plus the datasets that ship inside the package — which are re-hashed on every resolve, and so were guarded but only reactively: a damaged file announced itself in the middle of the run that needed it. An unpinned artifact, or a pinned one that is not on this disk, is reported as **not checked** and counted apart from the passes. |
