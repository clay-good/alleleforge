...
| **The k-mer seed prefilter never engages on its own** (`SEED_PREFILTER_AUTO_ENGAGES`) | Honest micro-benchmark finding, twice revised. It was calibrated at ~2–4× when selective (`k ≥ 5`), re-measured neutral, and is now a net cost at every budget: it prunes one native `evaluate_anchor` call, which is cheaper than its own `O(n)` pass costs to decide. It stays reachable as `scan_sequence(seed=True)` and stays parity-tested, because the proven-superset property is exact. ([`offtarget/_search.py`](src/alleleforge/offtarget/_search.py), R2/R411.) |
...
| **The default backbone is non-commercial, and the license gate enforces the use you declare** | Nucleotide Transformer v2 (500M) is **CC-BY-NC-SA-4.0** — loadable for research, *refused for commercial use* at load time. The gate cannot detect commercial use; it enforces what the operator declares, with `ALLELEFORGE_MODEL_USE=commercial` (or `model_use` in the config file), and the declaration is recorded in the run's provenance. It defaulted to research with no way for any shell to say otherwise until R487, so the refusal only ever reached a Python caller who already knew to ask for it. `aforge models list` shows each card's licence and which uses it permits. Real weights are never vendored. ([`model_zoo/cards/`](src/alleleforge/model_zoo/cards/), R1.) |
...
| **A `ReferenceGenome` is thread-safe to share; cohort workers still get their own** | `pyfaidx` has a shared file position, so concurrent reads on one handle race. The web server's sync handlers run in a threadpool over one shared reference, so each read is guarded by a per-instance lock (covering the read, not the CPU-bound design that follows). The cohort path instead hands each worker its own handle via a `reference_factory` — safe *and* parallel I/O. Parallel in fact as well as in structure: the long native kernels release the GIL (`Python::detach`), without which four workers spent their time queueing for the interpreter and `--max-workers 4` delivered 1.6x rather than ~3x. ([`genome/reference.py`](src/alleleforge/genome/reference.py).) |
...
and the trained adapters are research-only — all refused for the use the operator *declares* with
`ALLELEFORGE_MODEL_USE=commercial`, since nothing here can detect one), **requires explicit
...
| `aforge offtarget <spacer>` | Standalone population/haplotype-aware off-target search. `--scorer cfd|mit|cfd-cas12a` selects the specificity scorer. `--cache` reuses an identical reference scan across runs; `--genome-index` anchors PAMs through a persistent memory-mapped FM-index. Both are opt-in and neither changes a result. Cached reports are checksummed and re-checked on read, so an entry altered on disk is refused rather than served — this store holds the safety finding itself, and an edit that stays valid JSON would otherwise turn a perfect-match off-target into a clean guide. |
...
| `aforge verify <result.json \| result.provenance.json>` | Check a result's provenance is complete — it names every model and dataset used (checked *against the result*: a menu names a model for each chemistry it ranks, and a scoring matrix a candidate names must appear in `datasets`) and carries seed, version and config — and, with `--cache-dir`, re-hash each pinned checkpoint and dataset found there against the recorded hash. Exits non-zero on incomplete provenance or a hash mismatch: provenance as a *checkable contract*, not a record. Takes either shape `design` produces — the result JSON, or the bare `.provenance.json` sidecar written beside it, which for `--format tsv`, `parquet`, `html` and `pdf` is the only machine-readable provenance a run leaves behind. The sidecar records *how* the run was configured and not *what* it was asked: re-deriving a result needs the variant too, which lives in the result itself (a column in the flat tables, the header of the rendered ones) and is deliberately absent from a file meant to be passed around. **Without `--cache-dir` no bytes are re-hashed, and the command says so**: completeness and artifact integrity are two different claims and only one of them is free. |
| `aforge data list` / `show <name>` | Inspect the dataset registry (versions, licenses, provenance). |
| `aforge models list` / `show <name>` | Inspect the model zoo: licence, intended use, out-of-scope use, known failure modes, and whether the checkpoint is here. The five weight-free defaults report **bundled** rather than absent — they ship as code and need no checkpoint, and reporting them as missing told a reader the scorer behind their own menu was unavailable. |
| `aforge cache verify` | Check the four on-disk stores a run trusts: two holding *work* it reuses — the cross-run off-target report cache and the persistent FM-index cache. Each already knew how to detect a corrupted entry and neither could be *asked*: the report cache re-checks its checksum only when a run happens to read that entry, and the index's reconstruct-and-re-hash check — the only one that catches an alteration leaving the length intact, and so the only one that catches an index reporting positions that are not occurrences — was a Python method no shell could call. `--deep` runs it; without it the output says the indexes got their structural checks only, rather than reporting them as verified. Exits non-zero naming any entry that fails; nothing is repaired or deleted, since both stores are content-addressed and removing a named entry is always safe. The other two hold pinned *artifacts* — the dataset and checkpoint caches, plus the datasets that ship inside the package — which are re-hashed on every resolve, and so were guarded but only reactively: a damaged file announced itself in the middle of the run that needed it. The content-addressed namespaces are read off disk rather than named, so a store the command has never heard of is swept too — the first version named the off-target reports and walked past the embeddings cache beside them. An unpinned artifact, a pinned one that is not on this disk, and an entry in a namespace that stores no checksum are each reported as **not checked** and counted apart from the passes. |
...
| `aforge bench leaderboard <result.json…>` | Aggregate signed results into the model-card-gated leaderboard (Markdown/HTML). A result whose **primary metric is undefined** — a correlation over constant predictions, an AUROC over a single-class fold — is listed with the reason and **never ranked**: a rank is a claim that this model beat that one, and there is no number to support it. |
...
| `POST /api/jobs/design` → `GET /api/jobs/{job_id}` | Async job submit + status/progress/result. The served page submits every design this way and renders the report *and* each download from the finished job, so one click of **Design edits** is one run and every file a reader saves is a rendering of the same one |
...
| `GET /api/models` · `/api/models/{name}` | Inspect the model zoo: licence, intended use, out-of-scope use, known failure modes, checkpoint presence |
