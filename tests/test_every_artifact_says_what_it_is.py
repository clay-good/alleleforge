...
#: Every function in `src/` that writes a file, mapped to what it produces. The
#: enumeration above is written by hand — the docstring says so, and a fifth artifact
#: slipped past it for exactly that reason — so this derives the *population* from the
#: code and requires each member to be accounted for. A new writer fails here until
#: someone says which artifact it makes and whether the rule reaches it.
#:
#: Keyed on the enclosing function rather than a line number, so moving code does not
#: churn the list while adding a writer does.
WRITERS: dict[str, str] = {
    "alleleforge/cli/main.py::design": "the design report in every --format; covered above",
    "alleleforge/cli/main.py::batch": "the cohort summary TSV; covered above",
    "alleleforge/cli/main.py::bench_run": "the signed benchmark result; EXEMPT, see above",
    "alleleforge/cli/main.py::bench_leaderboard": "the leaderboard; covered above",
    "alleleforge/cli/main.py::_write_provenance_sidecar": "EXEMPT, see above",
    "alleleforge/design/cohort.py::design_many": "the run manifest header; covered above",
    "alleleforge/design/cohort.py::_atomic_write_text": "the per-item menu JSON; EXEMPT",
    # Not artifacts a user reads and acts on — the rule is about documents that carry a
    # finding, and these carry none.
    "alleleforge/cache.py::put_bytes": "a cache entry, addressed by content hash and "
    "read only by this tool; it is a copy of bytes it was given, not a document",
    "alleleforge/genome/index.py::_publish": "FM-index shards and their metadata: "
    "a binary acceleration structure with no findings in it, byte-identical in effect "
    "to not having it. `_build_to_disk` publishes each part through this one writer "
    "(temp file, then rename), which is why the entry moved here from it",
    "alleleforge/viz/figures.py::reference_bias_data": "the figure's input data, "
    "committed beside the figure it generates and regenerated byte-identically",
    "alleleforge/viz/figures.py::render_all_figures": "the committed docs figures, each "
    "of which states its own provenance in the caption the page renders around it",
}


def _writers_in(path: Path, root: Path) -> set[str]:
    """Return `path::function` for each file-writing call site in one module."""
    import ast
...
def _file_writers() -> set[str]:
    """Return every file-writing call site in `src/`, as `path::function`."""
    root = Path(__file__).resolve().parents[1] / "src"
    found: set[str] = set()
    for path in sorted(root.rglob("*.py")):
        found |= _writers_in(path, root)
    assert len(found) > 5, f"found {found}; the AST walk is broken, not the code"
    return found


def test_every_file_writer_in_the_library_is_accounted_for() -> None:
    """The population, derived — not the list somebody last remembered to extend."""
    missing = sorted(_file_writers() - set(WRITERS))
    assert not missing, (
        f"these functions write files and no one has said what they produce: {missing}. "
        "Add each to WRITERS with the artifact it makes, and either cover it in the "
        "checks above or record it in EXEMPT with the reason the rule does not reach it."
    )


def test_no_writer_entry_outlives_its_function() -> None:
    """An entry for a writer that is gone hides the next one that appears."""
    stale = sorted(set(WRITERS) - _file_writers())
    assert not stale, f"WRITERS names functions that no longer write files: {stale}"
